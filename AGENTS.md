...
- Red-defense counter sequence (2026-08-06, revised): the blue attack's `BlueAttackHit` notify triggers the defender's `RedDefense` (not played at resolution start); after RedDefense ends the defender plays `GoldCounter`; damage and `Hurt` on the target apply at the `GoldCounterHit` notify — or at `BlueAttackHit` when a 2-stack enhanced blue breaks the defense.
