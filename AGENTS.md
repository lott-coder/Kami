...
- Fallback conventions (runtime, not USTRUCT): empty `BlockFail`/`DodgeFail`/`ChargeInterrupted` → play `Hurt`; block success plays `BlockSuccess` (parry) first, then `GoldCounter` via `PlayAnimThenReaction` + `OnActionMontageEnded`.
- Red-defense counter sequence (2026-08-06): player `RedDefense` and enemy `BlueAttack` play simultaneously, then player `GoldCounter` after RedDefense ends and enemy `Hurt` after BlueAttack ends; mirrored for enemy red-defense counters. Sequencing uses per-side pending reactions (`bPlayerReactionPending`/`bEnemyReactionPending` + `OnActionMontageEnded`).
