...
- Tutorial hint presentation (2026-08-10): `WBP_CombatHUD` optional `TutorialHintPanel` wraps `TutorialHintText`. Optional widget animations (`BindWidgetAnimOptional`, names must match): `TutorialHintInAnim` (full in at round entry), `TutorialHintClashInAnim` (clash hint special entry — panel appears from the LEFT side of the screen; mid-round trigger; falls back to `TutorialHintInAnim` if unconfigured), `TutorialHintOutAnim` (full out when the player confirms an action, a clash ends, or the battle ends; finished callback collapses the panel), `TutorialHintClashOutAnim` (clash hint special exit — panel leaves toward the LEFT, tracked via `bCurrentHintIsClash`; falls back to `TutorialHintOutAnim` if unconfigured). There is NO text-only/consecutive-background behavior — every hint is a full In/Out cycle. ALL render values (translation/scale/opacity/pivot) are defined exclusively by WBP animation keyframes — C++ never pre-sets or snaps render values, it only toggles widget visibility and plays/stops the animations; missing animations fall back to instant show/hide; all timings live in the WBP animations.
