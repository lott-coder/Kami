...
- 资产路径：输入资产放 `Content/Input`，HUD 放 `Content/UI`；C++ 放 `Hole/Source/Hole/Public|Private` 对应子目录。
...
| `Hole/Content/UI/WBP_CombatHUD.uasset` | HUD 皮肤（父类 `UCombatHUDWidget`） | 新建（编辑器） |
...
1. `Content` 下新建文件夹 `UI`。
2. 在 `Content/UI` 右键 → User Interface → Widget Blueprint；Parent Class 搜索 `CombatHUDWidget` 选择 `UCombatHUDWidget`；命名 `WBP_CombatHUD`。
...
```bash
git add Hole/Content/Input Hole/Content/UI Hole/Content/Blueprint/Character/Roles/Dale/BP_Dale.uasset Hole/Content/Blueprint/Character/Enemies/Satan/BP_Satan.uasset
