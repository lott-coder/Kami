...
7. **碰撞攻击随机 Section（2026-08-06 追加）**：`ClashAttackBlueSections`/`ClashAttackWhiteSections`（FString，竖线分隔）配置可选 Section，`StartClash` 每次随机选一段播放；空则用 `ClashAttack*.SectionName`。
